...
def copy_lib(src, dst, name):
    if (config.check_option("config", "bundle_dylibs")) and is_dir(src + "/" + name + ".framework"):
        ...
        if (config.check_option("config", "bundle_xcframeworks")) and is_dir(src + "/simulator/" + name + ".framework"):
            ...
            cmd("xcodebuild", ["-create-xcframework", "-framework", dst + "/" + name + ".framework", "-framework", dst + "/simulator/" + name + ".framework", "-output", dst + "/" + name + ".xcframework"])
...
def extract_unicode(src, dst, is_no_errors=False):
    if "windows" == host_platform():
        run_as_bat_win_isolate(["chcp 65001", 'call 7z.exe x -y "' + src + '" "-o' + dst + '"', "exit"])
        return
...
def mac_correct_rpath_docbuilder(dir):
    ...
    mac_correct_rpath_binary(
        "./docbuilder", mac_icu_libs + ["UnicodeConverter", "kernel", "kernel_network", "graphics", "PdfFile", "XpsFile", "OFDFile", "DjVuFile", "HtmlFile2", "Fb2File", "EpubFile", "IWorkFile", "HWPFile", "doctrenderer", "DocxRenderer"]
    )
...
def mac_correct_rpath_desktop(dir):
    ...
    mac_correct_rpath_binary(
        "./editors_helper.app/Contents/MacOS/editors_helper", ["ascdocumentscore", "UnicodeConverter", "kernel", "kernel_network", "graphics", "PdfFile", "XpsFile", "OFDFile", "DjVuFile", "hunspell", "ooxmlsignature", "doctrenderer"]
    )
...
def linux_correct_rpath_docbuilder(dir):
    linux_set_origin_rpath_libraries(
        dir, ["docbuilder.jni.so", "docbuilder.c.so", "icuuc.so." + icu_ver, "doctrenderer.so", "graphics.so", "kernel.so", "kernel_network.so", "UnicodeConverter.so", "PdfFile.so", "XpsFile.so", "OFDFile.so", "DjVuFile.so", "DocxRenderer.so"]
    )
    return
...
def generate_sdkjs_plugin_list(dst):
    plugins_list = config.option("sdkjs-plugin").rsplit(", ") + config.option("sdkjs-plugin-server").rsplit(", ")
...
def convert_ios_framework_to_xcframework(folder, lib):
    ...
    cmd("xcrun", ["lipo", "-remove", "x86_64", "./" + lib + "_xc_tmp/iphoneos/" + lib + ".framework/" + lib, "-o", "./" + lib + "_xc_tmp/iphoneos/" + lib + ".framework/" + lib])
    cmd("xcrun", ["lipo", "-remove", "arm64", "./" + lib + "_xc_tmp/iphonesimulator/" + lib + ".framework/" + lib, "-o", "./" + lib + "_xc_tmp/iphonesimulator/" + lib + ".framework/" + lib])

    cmd("xcodebuild", ["-create-xcframework", "-framework", "./" + lib + "_xc_tmp/iphoneos/" + lib + ".framework/", "-framework", "./" + lib + "_xc_tmp/iphonesimulator/" + lib + ".framework/", "-output", lib + ".xcframework"])
